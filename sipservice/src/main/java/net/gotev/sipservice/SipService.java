...
public class SipService extends BackgroundService implements SipServiceConstants {
    ...
    private void startStack() {
        ...
        try {
            ...
            stringVector.add("turn.1.codecc.cn");
            epConfig.getUaConfig().setStunServer(stringVector);
            ...
            for (int i = 0; i < codecFmtpVector.size(); i++) {
                if (PROFILE_LEVEL_ID_HEADER.equals(codecFmtpVector.get(i).getName())) {
                    codecFmtpVector.get(i).setVal(PROFILE_LEVEL_ID_CUSTOM);
                    break;
                }
            }
            /*mediaFormatVideo.setFpsNum(30);
            mediaFormatVideo.setMaxBps(68000000);
            mediaFormatVideo.setAvgBps(68000000);*/

            vidCodecParam.setDecFmt(mediaFormatVideo);
